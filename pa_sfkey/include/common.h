...
namespace seeta {
	...
	class mybufferStreamfskey{
	public:
		mybufferStreamfskey(const void* data, int len){
			this->len = len;
			pos = 0;
		...
		virtual void read(void* buf, int size){
			if (pos + size > this->len){
				printf("mybufferStream..read ³¬³ö·¶Î§£º%d bytes\n", pos + size - this->len);
				return;
			}
		...
		virtual void read(void* buf, int unit, int count){
			read(buf, unit * count);
		}
