...
/** Initialize the facial landmark detection model.
  ...
  */
void CCFAN::InitModel(const void *model_string, int len)
{
	mybufferStreamfskey is(model_string, len);
  /*Open the model file*/
  //FILE *fp = fopen(model_path, "rb+");
  mean_shape_ = new float[pts_num_ * 2];
  is.read(mean_shape_, sizeof(float), pts_num_ * 2);

  /*Load the parameters of the first local stacked autoencoder network*/
  is.read(&lan1_size_, sizeof(int), 1);
  lan1_structure_ = new int[lan1_size_];
  is.read(lan1_structure_, sizeof(int), lan1_size_);
  ...
  for (int i = 0; i < lan1_size_ - 1; i++)
  {
	...
	is.read(lan1_w_[i], sizeof(float), layer_size);

    lan1_b_[i] = new float[lan1_structure_[i + 1]];
	is.read(lan1_b_[i], sizeof(float), lan1_structure_[i + 1]);
  }

  /*Load the parameters of the second local stacked autoencoder network*/
  is.read(&lan2_size_, sizeof(int), 1);
  lan2_structure_ = new int[lan2_size_];
  is.read(lan2_structure_, sizeof(int), lan2_size_);
  ...
  for (int i = 0; i < lan2_size_ - 1; i++)
  {
	...
	is.read(lan2_w_[i], sizeof(float), layer_size);

    lan2_b_[i] = new float[lan2_structure_[i + 1]];
	is.read(lan2_b_[i], sizeof(float), lan2_structure_[i + 1]);
  }
}
